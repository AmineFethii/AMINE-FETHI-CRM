...
export const Layout: React.FC<LayoutProps> = ({ 
  ...
  const [isInvoiceSubOpen, setIsInvoiceSubOpen] = useState(activeView.startsWith('invoicing'));
  ...
  const NavItem = ({ id, icon: Icon, label, hasChildren, isOpen, onToggle }: { id: string, icon: any, label: string, hasChildren?: boolean, isOpen?: boolean, onToggle?: () => void }) => (
    <div 
      ...
      className={`flex items-center justify-between px-4 py-3 rounded-lg cursor-pointer transition-all duration-200 group ${
        activeView === id || (hasChildren && activeView.startsWith(id))
  ...
  const SubNavItem = ({ id, label, icon: Icon }: { id: string, label: string, icon?: any }) => (
    <div 
      onClick={() => onNavigate && onNavigate(id)}
      className={`flex items-center gap-3 pl-12 pr-4 py-2 rounded-lg cursor-pointer transition-all duration-200 ${
        activeView === id 
        ...
        <nav className="flex-1 p-4 space-y-1 overflow-y-auto font-sans">
          ...
          {user.role === 'admin' && (
            <>
              ...
              {/* Invoicing with Submenu */}
              <NavItem 
                id="invoicing" 
                icon={Receipt} 
                label={t.invoicing} 
                hasChildren 
                isOpen={isInvoiceSubOpen}
                onToggle={() => setIsInvoiceSubOpen(!isInvoiceSubOpen)}
              />
              {isInvoiceSubOpen && (
                <div className="space-y-1 mt-1 animate-fade-in">
                  <SubNavItem id="invoicing-hub" label={t.invoiceHub} icon={LayoutDashboard} />
                  <SubNavItem id="invoicing-generator" label={t.invoiceGen} icon={Sparkles} />
                </div>
              )}
          ...
          {user.role === 'client' && cycleData && (
            <div className="bg-slate-800/50 rounded-xl p-3 border border-slate-700/50 group hover:border-blue-500/30 transition-all duration-300">
               ...
               <div className="h-1.5 w-full bg-slate-700 rounded-full overflow-hidden mb-2 relative">
                  <div className="h-full bg-gradient-to-r from-blue-600 to-blue-400 rounded-full transition-all duration-1000 ease-out" style={{ width: `${cycleData.percentage}%` }}></div>
               </div>
                 ...
                 <div className="absolute mt-3 w-80 md:w-96 bg-white rounded-xl shadow-2xl border border-slate-100 z-[110] overflow-hidden animate-fade-in-down right-0">
                   <div className="p-4 border-b border-slate-50 flex justify-between items-center bg-slate-50/50">
                     <div className="flex items-center gap-2">
                       ...
                       {unreadCount > 0 && <span className="text-xs bg-blue-100 text-blue-700 px-2 py-0.5 rounded-full font-medium">{unreadCount} {t.new}</span>}
                     </div>
                     {unreadCount > 0 && onMarkAllAsRead && <button onClick={onMarkAllAsRead} className="text-[10px] font-bold text-blue-600 hover:text-blue-700 hover:bg-blue-50 px-2 py-1 rounded transition-colors uppercase tracking-wider flex items-center gap-1"><CheckCheck size={12} /> {t.markRead}</button>}
                   </div>
                   <div className="max-h-[400px] overflow-y-auto">
                     {notifications.length > 0 ? (
                       <div className="divide-y divide-slate-50">
                         {notifications.map((notification) => (
                           <div key={notification.id} onClick={() => handleNotificationClick(notification.id)} className={`relative p-4 transition-all cursor-pointer flex gap-3 group ${!notification.read ? 'bg-blue-50/40 hover:bg-blue-50' : 'hover:bg-slate-50'}`}>
                             <div className={`mt-1.5 flex-shrink-0 w-2 h-2 rounded-full transition-colors ${!notification.read ? 'bg-blue-500' : 'bg-slate-200'}`}></div>
                             <div className="flex-1 pr-6">
                               <div className="flex justify-between items-start mb-1">
                                 <p className={`text-sm ${!notification.read ? 'font-bold text-slate-900' : 'font-medium text-slate-600'}`}>{notification.title}</p>
                                 <span className="text-[10px] text-slate-400 whitespace-nowrap mx-2">{new Date(notification.date).toLocaleDateString(undefined, { month: 'short', day: 'numeric' })}</span>
                               </div>
                               <p className={`text-xs leading-relaxed line-clamp-2 ${!notification.read ? 'text-slate-600' : 'text-slate-400'}`}>{notification.message}</p>
                             </div>
                             {!notification.read && <button onClick={(e) => { e.stopPropagation(); handleNotificationClick(notification.id); }} className="absolute top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-opacity bg-white shadow-md border border-slate-100 text-blue-600 p-1.5 rounded-full hover:bg-blue-50 transform hover:scale-110 right-3" title="Mark as read"><Check size={14} /></button>}
                           </div>
                         ))}
                       </div>
                     ) : (
                       <div className="p-8 text-center text-slate-400"><Bell size={24} className="mx-auto mb-2 opacity-20" /><p className="text-sm">{t.noNotifications}</p></div>
                     )}
                   </div>
                 </div>
               </>
